...
module.exports = {
  ...
  async execute(client, ctx) {
    ...
    try {
      const { url } = await fetchSocial("kill");
      const embed = new EmbedBuilder().setColor(0x4A3F5F).setDescription(`**${ctx.message.author.username}** killed **${target.username}**!`).setImage(url);
