...
module.exports = {
  ...
  async execute(client, ctx) {
    ...
    try {
      const { url } = await fetchSocial("smile");
      const embed = new EmbedBuilder().setColor(0x5865F2).setDescription(`**${ctx.message.author.username}** smiled at **${target.username}**!`).setImage(url);
