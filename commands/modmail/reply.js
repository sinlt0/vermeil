// ============================================================
//  commands/modmail/reply.js
//  Reply to the current modmail thread (visible to user)
// ============================================================
...
module.exports = {
  name:             "mreply",
  description:      "Reply to the current modmail thread.",
  category:         "modmail",
  aliases:          ["r"],
  usage:            "<message>",
  cooldown:         2,
  ...
  async execute(client, ctx) {
    ...
    // Send to user
    await relayToUser(client, user, guild, message, staff, false);
    ...
    // Delete command message for prefix
    if (ctx.type === "prefix") await ctx.message.delete().catch(() => {});
    else await ctx.interaction.reply({ content: "✅ Reply sent.", ephemeral: true });
