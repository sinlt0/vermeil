...
module.exports = {
  ...
  async execute(client, ctx) {
    ...
    if (!query) return message.reply("❌ Usage: `$im <character name>`");
    ...
    if (!char) {
      return message.reply({ embeds: [new EmbedBuilder()
        .setColor(0xED4245)
        .setDescription(`❌ **${query}** not found in the character database.\nTry `$cl \`${query}\`` to check exact name.`)] });
    }
    ...
    // Calc claim rank (position by globalClaimCount desc)
    ...
    const embed = new EmbedBuilder()
      .setColor(char.type === "waifu" ? 0xFF69B4 : 0x4169E1)
